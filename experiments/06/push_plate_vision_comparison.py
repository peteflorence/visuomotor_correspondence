...
logs_dir_path = "/home/manuelli/data_2/imitation/logs/push_plate"

config_yaml = os.path.join(imitation_src_dir, "experiments", "06", "06_mlp_stateless_position.yaml")
config = spartan_utils.getDictFromYamlFilename(config_yaml)

logs_config_yaml = os.path.join(spartan_source_dir,  "modules/imitation_agent/config/task/push_plate.yaml")
...
NUM_REPEAT_TRIALS = 1
NUM_DEPLOYS_EACH = 10  # set to 0 to not deploy. We are using 100.
TRAIN = True  # set to False to not train

# DEBUG
NUM_DEPLOYS_EACH = 0

if NUM_DEPLOYS_EACH > 0:
    raise ValueError("not supported")

# --- baseline comparisons

DO_BLIND = False
DO_GT_POSE = False  # but toggle on for 200 below
...
DO_DD_VANILLA = False  # but toggle on for 200 below
DO_DD_SURF = False
DO_DD_soft_3d = False  # but toggle on for 200 below
DO_DD_hard_3d = False  # but toggle on for 200 below
...
for num_logs in [100]:
    ...
    config_yaml = os.path.join(imitation_src_dir, "experiments", "06", "06_mlp_stateless_position.yaml")
    config = spartan_utils.getDictFromYamlFilename(config_yaml)
    config["global_training_steps"] = 75000  # 75000
    config["num_downsampled_logs"] = num_logs

    # SAVE_TO = "pdc/imitation/trained_models/mlp_position/experiment_06-logs-" + str(num_logs)
    SAVE_TO = "pdc/imitation/trained_models/experiment_06"
    ...
    if DO_BLIND:
        ...
        for i in range(NUM_DEPLOYS_EACH):
            os.system("cd ../../evaluation && python move_to_box_evaluator.py  --se2 --mlp --seed " + str(
                i) + " --deploy-idx " + str(i) + " --net " + network_chkpt + " --logging_dir " + name)
    ...
    if DO_GT_POSE:
        ...
        for i in range(NUM_DEPLOYS_EACH):
            ...
            os.system("cd ../../evaluation && python move_to_box_evaluator.py  --se2 --mlp --seed " + str(
                i) + " --deploy-idx " + str(i) + " --net " + network_chkpt + " --logging_dir " + logging_dir)
    ...
    config["model"][
        "descriptor_net"] = "/home/peteflo/data/pdc/trained_models/imitation/move_to_box_se2_in_frame_1e-6lamda_10/010000.pth"
    ...
    config["surf_descriptors"] = False
    config["model"][
        "descriptor_net"] = "/home/peteflo/data/pdc/trained_models/imitation/move_to_box_se2_in_frame_1e-6lamda_3d_10-soft/010000.pth"
    ...
    config["model"][
        "descriptor_net"] = "/home/peteflo/data/pdc/trained_models/imitation/move_to_box_se2_in_frame_1e-6lamda_3d_10-hard/010000.pth"
